...
public:
	TFWWorkbench() : CppUserModBase()
	{
		...
		ModDescription = STR("Playmore SAYA Module Revision 2 -  based on [The Forever Winter]TFW-Workbench");
		ModAuthors = STR("- Joint collaboration by Terraru and Rythus");

		Output::send<LogLevel::Default>(STR("[SAYA] [R2] {} {} {} loaded.\n"), ModDescription, ModVersion, ModAuthors);
		...
		LuaMadeSimple::Lua* hook_lua) -> void override
	{
		...
		Output::send<LogLevel::Default>(STR("[SAYA] [R2] Registered Lua functions for mod\n"));
	}
...
private:
		...
		if (!m_cached_data_table.contains(tableName))
		{
			...
			catch (const std::out_of_range& ex)
			{
				Output::send<LogLevel::Error>(
					STR("[SAYA] [R2] Failed to cache DataTable: {}\n"),
					to_wstring(ex.what())
				);
				return nullptr;
			}
	...
	auto GetDataTableRowStruct(std::string tableName) -> UScriptStruct*
	{
		...
		{
			Output::send<LogLevel::Verbose>(
				STR("[SAYA] [R2] Caching RowStruct for DataTable: {}\n"), 
				to_wstring(tableName)
			);
			try
			{
			...
			{
				Output::send<LogLevel::Error>(
					STR("[SAYA] [R2] Failed to cache RowStruct for DataTable {}: {}\n"),
					to_wstring(tableName),
					to_wstring(ex.what())
				);
				return nullptr;
		...
		const std::wstring propertyName) -> void
	{
		if (auto* prop = CastField<FTextProperty>(property))
		{
			if (table.value.is_string())
			{
				...
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Set FText property '{}' to value: {}\n"),
		...
		else if (auto* prop = CastField<FStrProperty>(property))
		{
			if (table.value.is_string())
			{
				...
				*static_cast<FString*>(propertyPtr) = FString(propertyValue.c_str());
				...
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Set FString property '{}' to value: {}\n"),
					propertyName, propertyValue
				);
			}
		}
		...
		else if (auto* prop = CastField<FIntProperty>(property))
		{
			if (table.value.is_number())
			{
				...
				*static_cast<int32*>(propertyPtr) = static_cast<int32>(propertyValue);
				...
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Set int32 property '{}' to value: {}\n"),
					propertyName, propertyValue
				);
			}
		}
		else if (auto* prop = CastField<FFloatProperty>(property))
		{
			if (table.value.is_number())
			{
                ...
                double propertyValue = table.value.get_float();
				*static_cast<float*>(propertyPtr) = static_cast<float>(propertyValue);

				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Set float property '{}' to value: {}\n"),
					propertyName, propertyValue
				);
			}
		}
        else if (auto* prop = CastField<FNumericProperty>(property))
        {
            if (table.value.is_number())
            {
                bool propertyValue = table.value.get_number();
                *static_cast<uint8*>(propertyPtr) = static_cast<uint8>(table.value.get_number());

                Output::send<LogLevel::Verbose>(STR("[SAYA] [R2] Set byte property '{}' to value: {}\n"), propertyName, propertyValue);
            }
        }
		else if (auto* prop = CastField<FBoolProperty>(property))
		{
			if (table.value.is_bool())
			{
				...
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Set bool property '{}' to value: {}\n"),
					propertyName, propertyValue
				);
			}
		}
		else if (auto* prop = CastField<FSoftObjectProperty>(property))
		{
			if (table.value.is_string())
			{
				...
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Set {} property '{}' to value: {}\n"),
		...
		else if (auto* prop = CastField<FDoubleProperty>(property))
		{
			if (table.value.is_number())
			{
				...
				*static_cast<double*>(propertyPtr) = propertyValue;
				...
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Set double property '{}' to value: {}\n"),
					propertyName, propertyValue
				);
			}
		}
		else if (auto* prop = CastField<FEnumProperty>(property))
		{
			if (table.value.is_number())
			{
                ...
                *static_cast<uint8*>(propertyPtr) = static_cast<uint8>(table.value.get_number());
				...
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Set FEnumProperty property '{}' to value: {}\n"),
					propertyName, propertyValue
				);
			}
		}
		else if (auto* prop = CastField<FObjectProperty>(property))
		{
			...
			if (table.value.is_string())
			{
				...
				{
					Output::send<LogLevel::Verbose>(
						STR("[SAYA] [R2] Found object via path: {}\n"), propertyValue
					);
		...
		else if (auto* prop = CastField<FMapProperty>(property))
		{
			if (table.value.is_table())
			{
				...
				if (count == 0) return;
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Creating map with {} elements\n"),
					count
				);
		...
		else if (auto* prop = CastField<FArrayProperty>(property))
		{
			...
			{
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Set FArrayProperty '{}'\n"),
					propertyName
				);
				...
				Output::send<LogLevel::Verbose>(
					STR("[SAYA] [R2] Creating array with {} elements (elementSize={}, alignment={})\n"),
			...
			else if (table.value.is_number())
			{
				...
				if (structName == STR("Timespan"))
				{
					...
					*static_cast<int64_t*>(propertyPtr) = propertyValue;
					...
					Output::send<LogLevel::Verbose>(
						STR("[SAYA] [R2] Set Timespan property '{}' to value: {}\n"),
						propertyName, propertyValue
					);
				}
			}
	...
	static auto Lua_AddDataTableRow(const LuaMadeSimple::Lua& lua) -> int
	{
		if (!s_instance)
		{
			Output::send<LogLevel::Error>(STR("[SAYA] [R2] No instance available\n"));
			lua.set_bool(false);
			return 1;
		}
		...
		try
		{
			...
			{
				Output::send<LogLevel::Error>(
					STR("[SAYA] [R2] Invalid parameters. Expected: (string, string, table)\n")
				);
				lua.set_bool(false);
				return 1;
			}
			...
			if (!dataTable)
			{
				Output::send<LogLevel::Error>(STR("[SAYA] [R2] DataTable not found: {}\n"), to_wstring(tableName));
				lua.set_bool(false);
				return 1;
			}
			...
			if (!rowStruct)
			{
				Output::send<LogLevel::Error>(STR("[SAYA] [R2] DataTable RowStruct not found\n"));
				lua.set_bool(false);
				return 1;
			}
			...
			if (!newRow)
			{
				Output::send<LogLevel::Error>(STR("[SAYA] [R2] Failed to allocate memory for new row\n"));
				lua.set_bool(false);
				return 1;
			}
            ...
            if (!sourceRow)
            {
                Output::send<LogLevel::Error>(STR("[SAYA] [R2] Source row {} not found\n"), sourceRowName);
                lua.set_bool(false);
                return 1;
            }
            ...
            Output::send<LogLevel::Default>(STR("[SAYA] [R2] Adding row '{}'\n"), to_wstring(newRowName));
			...
			if (!actualRow)
			{
				Output::send<LogLevel::Error>(STR("[SAYA] [R2] Failed to find newly added row\n"));
				lua.set_bool(false);
				return 1;
			}
			...
			lua.for_each_in_table([&](LuaMadeSimple::LuaTableReference table) -> bool {
				...
				Output::send<LogLevel::Verbose>(STR("[SAYA] [R2] Processing field '{}', stack depth: {}\n"),
					propertyName, stackBefore);
				//Output::send<LogLevel::Verbose>(STR("[SAYA] [R2] Got property '{}'\n"), propertyName);
				FProperty* property = rowStruct->GetPropertyByNameInChain(propertyName.c_str());
				if (!property)
				{
					Output::send<LogLevel::Warning>(
						STR("[SAYA] [R2] Property '{}' not found, skipping\n"),
						propertyName
					);
					return false;
				}
				...
				{
					Output::send<LogLevel::Error>(
						STR("[SAYA] [R2] STACK IMBALANCE after '{}: before={}, after={}\n"),
			...
			Output::send<LogLevel::Default>(STR("[SAYA] [R2] Successfully added row '{}'\n"), to_wstring(newRowName));
	...
	static auto Lua_ConfigureDataTables(const LuaMadeSimple::Lua& lua) -> int
	{
		...
		{
			Output::send<LogLevel::Error>(
				STR("[SAYA] [R2] Expected 3 arguments, got {}\n"),
				argCount
			);
			lua.set_bool(false);
			return 1;
		...
		{
			Output::send<LogLevel::Error>(
				STR("[SAYA] [R2] Invalid parameter types. Expected: (string, string, string)\n")
			);
			lua.set_bool(false);
			return 1;
		}
		...
		{
			Output::send<LogLevel::Error>(
				STR("[SAYA] [R2] Parameters cannot be null or empty\n")
			);
			lua.set_bool(false);
			return 1;
		}
		...
		Output::send<LogLevel::Verbose>(
			STR("[SAYA] [R2] Configuring DataTable: {} | {} | {}\n"),
		...
		{
			Output::send<LogLevel::Error>(
				STR("[SAYA] [R2] Failed to configure data table: {} - {}\n"),
        ...
        {
            Output::send<LogLevel::Error>(STR("[SAYA] [R2] Failed to configure source row for data table: {} - {}\n"), to_wstring(tableName), sourceRowName);
            Output::send<LogLevel::Error>(STR("[SAYA] [R2] {}\n"), to_wstring(ex.what()));
            lua.set_bool(false);
            return 1;
        }
